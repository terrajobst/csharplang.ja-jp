...
属性クラス ([属性クラス](attributes.md#attribute-classes)) 1 つまたは複数で修飾された`Conditional`属性は、***条件付き属性クラス***します。 宣言されている条件付きコンパイル シンボルを使用した条件付き属性クラスが関連付けられているなりますその`Conditional`属性。 この例の場合:
