...
|                 |                 | 高精度の 10 進数: `decimal` |
|                 |                 | ブール値: `bool` |
|                 | 列挙型      | フォームのユーザー定義型 `enum E {...}` |
|                 | 構造体の型    | フォームのユーザー定義型 `struct S {...}` |
|                 | Null 許容型  | `null` 値を持つその他すべての値型の拡張子 |
| 参照型 | クラス型     | その他のすべての種類の最終的な基底クラス: `object` |
|                 |                 | Unicode 文字列。 `string` |
|                 |                 | フォームのユーザー定義型 `class C {...}` |
|                 | インターフェイス型 | フォームのユーザー定義型 `interface I {...}` |
|                 | 配列型     | 1 つと例については、複数次元`int[]`と `int[,]` |
...
| __カテゴリ__      | __ビット__ | __型__  | __範囲/有効桁数__ |
|-------------------|----------|-----------|---------------------|
...
| 代入または匿名関数 | `x = y`           | 代入 |
|                                  | `x op= y`         | 複合代入。サポートされる演算子は次のとおりです: `*=` `/=` `%=` `+=` `-=` `<<=` `>>=` `&=` `^=` <code>&#124;=</code> |
|                                  | `(T x) => y`      | 匿名関数 (ラムダ式) |
