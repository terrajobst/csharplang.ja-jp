...
これらのルールにより、共変のインターフェイスの使用状況を反変、タイプ セーフまたはします。 たとえば、オブジェクトに適用された
```csharp
...
厳密にはインターフェイスの単一継承 (継承チェーン内の各インターフェイスは、正確に 0 個または 1 つの直接基底インターフェイスを持つ)、メンバー検索の効果 ([メンバー ルックアップ](expressions.md#member-lookup))、メソッドの呼び出し ([メソッドの呼び出し](expressions.md#method-invocations))、アクセスおよびインデクサー アクセス ([インデクサー アクセス](expressions.md#indexer-access)) ルールはまったく同じクラスと構造体です。 同じ名前またはシグネチャを持つ派生メンバー以下より強い派生メンバーの非表示。 ただし、多重継承インターフェイスでは、2 つに、あいまいさが発生することがまたは以上の関連付けられていない基本インターフェイスが同じ名前またはシグネチャを持つメンバーを宣言します。 このセクションでは、このような状況のいくつかの例を示します。 すべてのケースで、あいまいさを解決するのには明示的なキャストを使用できます。
...
多重継承インターフェイスで非表示の直感的な規則は単にこの: すべてのアクセス パスで非表示は、任意のアクセス パスでメンバーが非表示の場合。 からのアクセス パス`IDerived`に`ILeft`に`IBase`を非表示に`IBase.F`からのアクセス パスで、メンバーは非表示も`IDerived`に`IRight`に`IBase`します。
