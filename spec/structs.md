...
構造体の型がから継承された仮想メソッドをオーバーライドするときに`System.Object`(など`Equals`、 `GetHashCode`、または`ToString`)、構造体の型のインスタンス経由で仮想メソッドの呼び出しが発生するボックス化は発生しません。 これは、構造体は型パラメーターとして使用され、型パラメーターの型のインスタンスを通じて、呼び出しが発生した場合でも当てはまります。 例えば:
```csharp
using System;
