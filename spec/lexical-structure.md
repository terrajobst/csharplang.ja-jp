...
C# プログラムですべてのソース ファイルに準拠する必要があります、 *compilation_unit*構文文法の実稼働 ([コンパイル単位](namespaces.md#compilation-units))。
...
注: ANTLR 文法の表記は、次混乱を招く。 記述するときに、ANTLR`\'`の単一引用符現時点では`'`します。 記述するときと`\\`現時点では、1 つの円記号の`\`します。 そのため、単一引用符、文字、単一引用符で始まるリテラル文字の最初の規則を意味します。 11 個の可能な単純なエスケープ シーケンスと`\'`、 `\"`、 `\\`、 `\0`、 `\a`、 `\b`、 `\f`、 `\n`、 `\r`、 `\t`、 `\v`.
